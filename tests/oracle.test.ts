...
describe("oracle", () => {
  ...
  it("oracle ownership test #3", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #4", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #5", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #6", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #7", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #8", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #9", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #10", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });
  ...
  it("oracle ownership test #12", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #13", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #14", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #15", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #16", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #17", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #18", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #19", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #20", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #21", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #22", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #23", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #24", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #25", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #26", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #27", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #28", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #29", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #30", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #31", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #32", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #33", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #34", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #35", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #36", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #37", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });

  it("oracle ownership test #38", () => {
    const { result } = simnet.callReadOnlyFn("oracle", "get-owner", [], deployer);
    expect(result).toBeOk(expect.anything());
  });
