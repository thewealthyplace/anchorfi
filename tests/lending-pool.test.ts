...
describe("lending-pool", () => {
  ...
  it("error handling scenario #19 on lending-pool", () => {
    const { result } = simnet.callPublicFn("lending-pool", "borrow", [Cl.uint(0), Cl.uint(0)], wallet1);
    expect((result as any).type).toBe("err");
  });
  ...
  it("configure test #10", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #11", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #12", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });
  ...
  it("configure test #14", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #15", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #16", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #17", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #18", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #19", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #20", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #21", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #22", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #23", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #24", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #25", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #26", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #27", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #28", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #29", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #30", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #31", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #32", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #33", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #34", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #35", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #36", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #37", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #38", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #39", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #40", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #41", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #42", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });

  it("configure test #43", () => {
    const { result } = simnet.callPublicFn("lending-pool", "configure",
      [Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer), Cl.principal(deployer)], wallet1);
    expect(result).toBeErr(Cl.uint(400));
  });
